        ...
        return (
            <div className={styles.container}>
                ...
                {this.state.isFlipped ?
                    <img src="/d_su.svg" alt="Sign in" className={styles.img} style={{ height: this.state.height, width: this.state.width }} /> :
                    <img src="/d_si.svg" alt="Log in" className={styles.img} style={{ height: this.state.height, width: this.state.width }} />}
                {this.state.changeEmail_PhoneNumber ?
                    ...
                    >
                        ...
                        <p className={styles.cvTitle}>Enter new email</p>
                        <div className={styles.inputCont_}>
                            <div className={styles.inputContHalf} style={{ width: '65%' }}>
                                ...
                                <input
                                    ...
                                    onChange={(v) => { this.setState({ newEmail: v.target.value }) }} />
                            </div>
                            <div className={styles.button1} style={{ width: '30%' }} onClick={() => {
                                if (this.state.newEmail.length == 0)
                                    this.setState({ error: true, errorMessage: 'Email must be valid' })
                                else
                                    this.setState({ changeEmailLoader: true }, () => {
                                        changeEmailOrPhoneNumber.call(this, 'email', this.state.userDetails.userID, '', this.state.newEmail)
                                    });
                            }}>
                    ...
                    this.state.displayVerification ?
                        ...
                        >
                            ...
                            {this.state.phoneVerified ?
                                ...
                                <>
                                    ...
                                    {this.state.phoneNumberTimer == 0 ?
                                        <p className={styles.cvTitle} >Didnt get the code yet? <span className={styles.cursor} style={{ color: "#005BEA", display: 'inline', fontFamily: 'Gilroy-Medium' }} onClick={() => {
                                            if (!this.state.verificationPhoneLoader)
                            ...
                            {this.state.emailVerified ?
                                <></> :
                                <>
                                    <p className={styles.cvTitle} style={{ marginTop: '10px' }}>Enter the verification code sent to the email <span style={{ color: "#4EB848", display: 'inline-block' }}>{this.state.userDetails.email}</span></p>
                                    <div className={styles.inputCont_} style={{ marginBottom: '5px', }}>
                                        <div className={styles.inputContHalf} style={{ width: '65%', marginBottom: '0px' }}>
                                            <CgKeyboard color={'#4EB848'} size={20} style={{ marginRight: '10px', marginLeft: '10px' }} />
                                            <input
                                                ...
                                                onChange={(event) => { this.setState({ emailVerificationCode: event.target.value }) }} />
                                        </div>
                                        <div className={styles.button1} style={{ width: '30%' }}
                                            onClick={() => {
                                                if (!this.state.verificationEmailLoader) {
                                                    if (this.state.emailVerificationCode.length != 6)
                                                        this.setState({ errorMessage: 'Code must be exactly 6 digits long', error: true })
                                                    else
                                                        this.setState({ verificationEmailLoader: true }, () => {
                                                            sendVerification.call(this, this.state.userDetails.userID, 'email', 'check', this.state.emailVerificationCode, this.state.userDetails.phoneNumber, this.state.userDetails.email, this.state.userDetails.firstName,);
                                                        });
                                                }
                                            }}>
                                            {this.state.verificationEmailLoader ?
                                                <Loader
                                                    type="TailSpin"
                                                    color={WHITE}
                                                    height={'20px'}
                                    ...
                                    {this.state.emailTimer == 0 ?
                                        <p className={styles.cvTitle} >Didnt get the code yet? <span className={styles.cursor} style={{ color: "#005BEA", display: 'inline', fontFamily: 'Gilroy-Medium' }}
                                            onClick={() => {
                                                if (!this.state.verificationEmailLoader)
                                                    this.resend('email')
                                            }}>Resend</span></p> :
                                        <p className={styles.cvTitle}>Resend in<span style={{ color: "#005BEA", display: 'inline', fontFamily: 'Gilroy-Medium' }}>{` 0:${('0' + this.state.emailTimer).slice(-2)}`}</span></p>
                                    }
                                </>}
                            ...
                            <a className={styles.fp_} onClick={() => { this.setState({ changeEmail_PhoneNumber: true, error: false }) }}><p className={styles.fp} style={{ fontSize: 'smaller' }}>Change phone number or email</p></a>
                            <div className={styles.inputCont_d} style={{ marginTop: '30px', }}>
                                <p className={styles.cu_} style={{ margin: '0px' }}>Running into an issue? <a href='/contact_us'><span className={styles.cu} style={{ color: '#4EB848' }}>Contact us</span></a></p>
                                <p
                                    className={styles.cu}
                                    style={{ fontFamily: 'Gilroy-Semibold', color: '#FF0000', margin: '0px' }}
                                    onClick={() => {
                                        this.setState({ email: '', password: '', displayVerification: false, isFlipped: false, changeEmail_PhoneNumber: false, error: false }, () => {
                                            signOut.call(this, true);
                                        });
                        ...
                        <ReactCardFlip isFlipped={this.state.isFlipped} flipDirection="horizontal" cardZIndex={2} containerStyle={{ marginRight: '5vw' }}>
                                ...
                                <a className={styles.logo} href='/'><Logo color={'#4EB848'} /></a>
                                {
                                    this.state.forgotPassword ?
                                        ...
                                        <>
                                                ...
                                                onClick={() => {
                                                    if (!this.state.loading) {
                                                        if (this.state.email == '')
                                                            ...
                                                            signIn.call(this, this.state.email, this.state.password, 'driver');
                                                    }
                                                }} >
                                                {this.state.loading ?
                                                    <Loader
                                            ...
                                            <a className={styles.fp_}><p className={styles.fp} onClick={() => { this.setState({ forgotPassword: true, errorMessage: '', error: false }) }}>Forgot password?</p></a>
                                            <p className={styles.cu_}>Running into an issue? <a href='/contact_us'><span className={styles.cu}>Contact us</span></a></p>
                                            <p className={styles.ca}>Don't have an account</p>
                                            <a className={styles.caC}
                            ...
                            >
                                ...
                                {this.state.error ?
                                    ...
                                    <></>}
                                <a className={styles.button1} onClick={() => {
                                    if (!this.state.loading) {
